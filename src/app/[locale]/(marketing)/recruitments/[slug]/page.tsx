...
interface RecruitmentPageProps {
  ...
  params: Promise<RecruitmentPageParams>;
}
...
export default async function RecruitmentPage({
  ...
  const recruitment: Recruitment | null = await fetchRecruitmentBySlug(
    slug,
    locale
  );
  if (!recruitment) {
  ...
  const localizedSlugs =
    recruitment?.localizations?.reduce(
  ...
  const types = recruitment?.recruitments_types
    ...
    .join(" • ");
  const subtitle =
    types || (locale === "fr" ? "Opportunité" : "Opportunity");
...
export async function generateMetadata(
  ...
  const plain = recruitment.content
    ? recruitment.content
        .replace(/<[^>]+>/g, " ")
        .replace(/\s+/g, " ")
        .trim()
  ...
  return {
    ...
    description,
    openGraph: { title: recruitment.title, description },
  };
}
