...
import {
  Article,
  PartnersResponse,
  DirectorWord as DirectorWordType,
  HomePage,
} from "@/types/types";
...
export default async function Home({ params }: HomeProps) {
  ...
  const homePage: HomePage = await fetchContentType(
    "home-page",
    {
      populate: {
        background: {
  ...
  const youtube = await fetchContentType(
    "youtube-channel",
    {
      populate: "*",
    },
  ...
  const directorWordResponse = await fetchContentType(
    "director-word",
    {
      populate: {
        director: {
  ...
  const membersCount = Array.isArray(membersResponse?.data)
    ...
    : 0;
  const publicationsCount = Array.isArray(publicationsResponse?.data)
    ? publicationsResponse.data.length
    : 0;
  const projectsCount = Array.isArray(projectsResponse?.data)
    ? projectsResponse.data.length
    : 0;
  const teamsCount = Array.isArray(teamsResponse?.data)
  ...
  return (
    <main className="min-h-screen">
      ...
      <YouTubeChannel
        channelUrl={youtube?.channel_url}
        videos={youtube?.videos || []}
      />

      {/* Partners Section */}
      <Partners
        partners={partnersResponse?.data?.partner || []}
        locale={locale}
      />
    </main>
  );
