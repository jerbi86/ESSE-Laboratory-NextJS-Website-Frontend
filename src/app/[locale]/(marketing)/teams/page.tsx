...
export default async function TeamsIndex({ params }: TeamsIndexProps) {
  ...
  const texts = locale === 'en'
    ? { title: 'Research Teams', subtitle: 'Discover our teams and their projects' }
    : { title: 'Équipes de recherche', subtitle: 'Découvrez nos équipes et leurs projets' };

  return (
    <main className="min-h-screen">
      <ClientSlugHandler localizedSlugs={{ en: 'teams', fr: 'teams' }} />
      <Hero title={texts.title} description={texts.subtitle} size="lg" />
      <TeamsList teams={teams as any} locale={locale} />
    </main>
  );
}
